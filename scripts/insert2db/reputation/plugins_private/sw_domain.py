...
class SW_DOMAIN():
    def __init__(self, ID=0):
        self.name = 'SecureWorksDomain'
        self.ID = ID
        token = conf.get('secureware', 'token')
        self.URL = 'https://ws.secureworks.com/ti/v1/attackerdb-token/blackList?type=domain&listId=0&format=csv&token=' + token
        self.DataFilePath = DataDir + 'secureworks/domain.csv'
        self.header = [
            'WatchList',
            'HostAddress',
            'ReasonAdded',
    ...
    def delComment(self, s):
        ...
        for line in s.splitlines(True):
            if not line.startswith('"WatchList'):
                result += line
        return result
